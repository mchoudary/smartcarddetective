...
def visualise_scd_eeprom(port, fid = sys.stdout):
    """Retrieves the EEPROM trace from the SCD and parses the information showing information
    about an EMV transaction (if available)."""

    tmp = "scdtrace.tmp~"
    ftmp = open(tmp, 'w')
    serial_geteepromhex(port, ftmp)

    traces = parseSCDHexTrace(tmp)
    count = 1
    for trace in traces:
        fid.write("\nTransaction %d\n"%(count))
        strace = SCDTrace(trace)
        fid.write("Number of messages: %d\n\n"%(len(strace.items)))
        fid.write(strace.pretty_print())
        count = count + 1
...
def main():
    ...
    parser.add_argument('--terminal', action = 'store_true',  help='run the terminal application available on the SCD and log transaction on EEPROM')
    parser.add_argument('--logt', action = 'store_true',  help='log a card-reader transaction. You might need to\
            run this command twice if the reader resets the card at the beginning of the transaction.')
    ...
    parser.add_argument('--vet', nargs = '?', type = argparse.FileType('w'), const = sys.stdout,
            default = False, metavar = 'filename',
            help='visualise the EEPROM traces from SCD (default to stdout)')
    ...
    if args.vet != False:
        try:
            ...
            visualise_scd_eeprom(args.port, args.vet)
            print "Done"
        except:
            print "Error occurred"
            raise
